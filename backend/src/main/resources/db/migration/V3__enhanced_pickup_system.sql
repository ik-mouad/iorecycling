...
-- Ajout des colonnes à la table pickup
ALTER TABLE pickup 
...
CREATE TABLE pickup_item (
    id BIGSERIAL PRIMARY KEY,
    pickup_id BIGINT NOT NULL REFERENCES pickup(id) ON DELETE CASCADE,
...
CREATE INDEX idx_pickup_client_date ON pickup(client_id, date DESC);
CREATE INDEX idx_pickup_site ON pickup(site_id);
CREATE INDEX idx_pickup_type ON pickup(type);
...
UPDATE pickup SET site_id = 1 WHERE site_id IS NULL;
...
-- Pour le premier pickup recyclable (si il existe)
((SELECT id FROM pickup WHERE type = 'RECYCLABLE' LIMIT 1), 'Papier', 150.5, 2.5),
((SELECT id FROM pickup WHERE type = 'RECYCLABLE' LIMIT 1), 'Carton', 200.0, 1.8),
((SELECT id FROM pickup WHERE type = 'RECYCLABLE' LIMIT 1), 'Plastique PET', 75.2, 4.2),
-- Pour le deuxième pickup recyclable
((SELECT id FROM pickup WHERE type = 'RECYCLABLE' LIMIT 1 OFFSET 1), 'Métal', 120.8, 6.5),
((SELECT id FROM pickup WHERE type = 'RECYCLABLE' LIMIT 1 OFFSET 1), 'Verre', 95.3, 0.8);
...
COMMENT ON COLUMN pickup.type IS 'Type d''enlèvement: RECYCLABLE, BANAL, DANGEREUX';
COMMENT ON COLUMN pickup.site_id IS 'Site de collecte (optionnel)';
